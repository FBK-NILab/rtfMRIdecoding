function poll_for_data_preproc_version1(SubjectID, sessionN, cfg)
...
%     cfg = [];
% end
cfg.inputDir='C:\Documents\RealTime\20150717ANSN\'; %'C:\Users\eust_abbondanza\Documents\realtime\20130429_19720216VLRZ\Ser0001\';
cfg.NrOfVols=305;
cfg.TimeOut=6.0;
...
%cfg.blockDurVol=8;
cfg.TR=2;
cfg.TRtoTake= 3; %3; added more because of the block duraton, should do convolution and calculate the delay !!!!!!
...
if ~isfield(cfg, 'maskpath')
    cfg.maskpath='C:\Documents\RealTime\20150717ANSN\';
end
...
switch cfg.Classifier
    case 1
        ...
        W = svc(A);
        fprintf('\nSVM classifier trained in %d sec...\n', toc(classif_time));
    case 2
...
while 1 %length(files)
 ...
 %%%%%%%%%%%%%%%%%   target=dir(fullfile(cfg.inputDir,name_template));
    target=dir(fullfile(cfg.output, sprintf('Ser%04d',sessionN), name_template));
    if isempty(target)
        fprintf('\nNo new data\n');
        time=toc(GrabVol);
        ...
        fname=fullfile(cfg.output,sprintf('Ser%04d',sessionN),name_template);
    ...
    %    threshold.nsamples = numTotal + S.numEchos - 1;
        ...
        testSample=(procSample(maskvol_vol>cfg.maskThreshold))';
        testSample=scaledata(testSample, 0, 1);
        plot(testSample)
        
        filename=sprintf('prepScan_%i.nii', sprintf('Ser%04d',sessionN), numProper);
        V=[];
        V.fname=fullfile(cfg.output, filename);
        %     V.voxdim=S.voxdim;
        V.dt=[4 0];
        V.x=S.vx;
        V.y=S.vy;
       ...
       % testSample=double(procScan);
        ...
        switch cfg.Classifier
            ...
            case 2
                ...
                predicted_labels1=vertcat(predicted_labels1, estimate);
                test_label2=glmval(cfs, testSample, 'logit')
                ...
                predicted_labels2=vertcat(predicted_labels2, estimate);
        end
