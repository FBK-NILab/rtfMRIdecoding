function poll_for_data_preproc_ses1_version1(SubjectID, sessionN, cfg)
...
%     cfg = [];
% end
% cfg.inputDir='C:\Users\eust_abbondanza\Documents\realtime\20130429_19720216VLRZ\Ser0001\';
% cfg.NrOfVols=100;
% cfg.TimeOut=6.0;
...
while 1 %length(files)
    ...
    tic
    pause(1.0);
    name_template=sprintf('Analyze%05d.hdr', count);
    %start timer
    ...
    if isempty(target)
        ...
        V=[];
        V.fname=fullfile(cfg.output,sprintf('Ser%04d',sessionN), filename);
        %     V.voxdim=S.voxdim;
        V.dt=[4 0];
        V.x=S.vx;
        V.y=S.vy;
