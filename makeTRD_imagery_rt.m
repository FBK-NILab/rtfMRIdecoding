function makeTRD_imagery_rt(subjectID, sessionNumber)
...
info.nRepeat=16;
info.nPictures=20; %20; %the number of stimuli in each condition type, e.g. 10 people and 10 cars
            ...
            for iCondition=1:info.nCueTypes
                %sprintf('carorperson=%d', iCondition*iRepeat)
                        ...
                        ThisTrial.code = ASF_encode( [iRepeat-1 iCondition-1], info.factorialStructure);
                            ...
                            ThisTrial.tOnset = 0;
                            ThisTrial.userDefined = info.Stop;
                            
                            pages = [info.cuePictures(iCondition)];
                            durations=[info.cueDuration];
                            for p = 1:info.nPictures-3
                                
                                    pages=[pages, info.emptyPicture, info.emptyPicture];
...
TrialDefinitions(1).durations=repmat(16, 1, TrialDefinitions(1).nPages);
...
trdName = sprintf('%s_Im-%d.trd', subjectID, sessionNumber);
%THIS OPENS A TEXT FILE FOR WRITING
fid = fopen(trdName, 'w');
%DESIGN
